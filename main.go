...
type Training struct {
	...
	LenStep      float64       // длина шага или гребка в м
...
func (t Training) meanSpeed() float64 {
	return t.distance() / (t.Duration.Seconds() / 3600) // переводим секунды в часы
}
...
func (w Walking) Calories() float64 {
	speed := w.meanSpeed() * KmHInMsec // переводим скорость в м/с
	return ((CaloriesWeightMultiplier*w.Weight +
		(math.Pow(speed, 2)/((w.Height/CmInM)*0.01))*CaloriesSpeedHeightMultiplier*w.Weight) *
		(w.Duration.Seconds() / 3600)) // переводим секунды в часы
}
...
func (s Swimming) meanSpeed() float64 {
	return float64(s.LengthPool*s.CountPool) / MInKm / (s.Duration.Seconds() / 3600) // переводим секунды в часы
}
...
func (s Swimming) Calories() float64 {
	speed := s.meanSpeed()
	return (speed + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Weight * (s.Duration.Seconds() / 3600) // переводим секунды в часы
}
